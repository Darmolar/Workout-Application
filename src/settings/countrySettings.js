...
import { View, Text, StyleSheet, Dimensions, TouchableOpacity, Switch, TextInput, Pressable, ActivityIndicator } from 'react-native';
...
export default function countrySettingsScreen({ navigation }){
    ...
    const selectCountry = async (name) => {
        setUserDetails({ ...userDetails, country: name });
    }
        ...
        await fetch('https://quantumleaptech.org/getFit/api/v1/user/update', {
                    ...
                    .then(async (json) => {
                        ...
                        if(json.status === true){  
                            ...
                            userDetails.country = json.data.country; 
                    ...
                    <ScrollView>
                        { 
                            countries.map((item, index) => {
                                return (
                                    <Pressable key={index} style={styles.listContainer} onPress={() => selectCountry(item.name)}>
                                        <Text style={styles.listContainerText}>{item.name}</Text>  
                                        <View style={ userDetails.country == item.name ? [styles.radioButon, { backgroundColor: '#000' }] : styles.radioButon}></View>
                                    </Pressable> 
                                )
                            })
                        }       
